    ...
    class ChatbotApp {
        ...
        createStyles() {
            ...
            style.textContent = `
                ...
                * {
                ...
                .floating-robot-btn-minimized:hover {
                    background: linear-gradient(135deg, #5a67d8 0%, #6b46c1 100%) !important;
                    transform: translateY(-2px) !important;
                    box-shadow: 0 12px 40px rgba(0, 0, 0, 0.2) !important;
                }
                ...
                .chat-subscribe-btn:hover .subscribe-icon {
                    color: #f57c00;
                }
        ...
        createRequiredElements() {
            ...
            if (!document.getElementById('floating-robot-btn')) {
                ...
                robotBtn.innerHTML = '<i class="mdi mdi-plus"></i>';
        ...
        renderFloatingMenu() {
            ...
            if (this.bShowFloatingMenu && this.openChatRoomId === null) {
                ...
                menuContainer.className = 'floating-menu mobile-app';
                robotBtn.classList.add('hidden');

                // 샘플 미리보기/시간/뱃지 데이터
                const chatList = [
                  { id: 'construction-safety', label: '건설안전 A.I', icon: 'mdi-robot', theme: 'construction-safety', preview: '안전 및 대책 관련 문의', badge: 0 },
                  { id: 'risk-assessment', label: '위험성평가 A.I', icon: 'mdi-shield-check', theme: 'risk-assessment', preview: '시스템사용방법 관련 문의', badge: 0 },
                  { id: 'tax-ai', label: '세금계산서 A.I', icon: 'mdi-calculator', theme: 'tax-ai', preview: '세금계산서 관련 문의', badge: 0 },
                ...
                menuContainer.innerHTML = `
                  <div class="mobile-header">
                    <div class="status-bar">
                      <span class="status-time">${getCurrentTimeString()}</span>
                    ...
                    <div class="header-content">
                      <div class="profile-avatar">
                        <img src="images/logo/symbol-kosha.png" alt="profile" />
                      </div>
                      <div class="header-text">
                        <h1 class="header-title">채팅</h1>
                        <p class="header-subtitle">스마트위험성평가 Chatbot</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="mobile-content">
                    ...
                    <div class="mobile-chat-list">
                      ${chatList.map((item, index) => {
                        ...
                        const lastTime = getLastMessageTime(item.id);
                        
                        if (item.isExternal) {
                          return `
                            <div class="mobile-chat-card" onclick="window.open('${item.link}', '_blank')" style="animation-delay: ${index * 0.1}s">
                              <div class="mobile-chat-avatar" data-theme="${item.theme}">
                                <i class="mdi ${item.icon}" style="font-size: 24px; color: #666;"></i>
                              </div>
                              <div class="mobile-chat-info">
                                <div class="mobile-chat-title-row">
                                  <span class="mobile-chat-title">${item.label}</span>
                                  <span class="mobile-chat-time">${lastTime}</span>
                                </div>
                                <div class="mobile-chat-preview">${item.preview}</div>
                              </div>
                            </div>
                          `;
                        } else {
                          return `
                            <div class="mobile-chat-card" onclick="window.chatbotApp.enterChatRoom('${item.id}')" style="animation-delay: ${index * 0.1}s">
                              <div class="mobile-chat-avatar" data-theme="${item.theme}">
                                <i class="mdi ${item.icon}" style="font-size: 24px; color: #666;"></i>
                                ${isMinimized ? '<span class="mobile-chat-indicator"></span>' : ''}
                              </div>
                              <div class="mobile-chat-info">
                                <div class="mobile-chat-title-row">
                                  <span class="mobile-chat-title">${item.label}</span>
                  ...
                  <div class="mobile-bottom-nav">
                    <div class="mobile-nav-item active">
                      <i class="mdi mdi-message-text mobile-nav-icon"></i>
                      <span class="mobile-nav-label">채팅</span>
                    </div>
                    <div class="mobile-nav-item">
                      <i class="mdi mdi-star-circle mobile-nav-icon"></i>
                      <span class="mobile-nav-label">구독</span>
                    </div>
                    <div class="mobile-nav-item">
                      <i class="mdi mdi-cog mobile-nav-icon"></i>
                      <span class="mobile-nav-label">설정</span>
                    </div>
                  </div>
                `;

                // 검색 기능 추가
                const searchInput = menuContainer.querySelector('.search-input');
                if (searchInput) {
                  searchInput.addEventListener('input', (e) => {
                    const searchTerm = e.target.value.toLowerCase();
                    const chatCards = menuContainer.querySelectorAll('.mobile-chat-card');
                    
                    chatCards.forEach(card => {
                      const title = card.querySelector('.mobile-chat-title').textContent.toLowerCase();
                      const preview = card.querySelector('.mobile-chat-preview').textContent.toLowerCase();
                ...
                // 하단 네비게이션 이벤트
                const navItems = menuContainer.querySelectorAll('.mobile-nav-item');
                navItems.forEach(item => {
                  item.addEventListener('click', () => {
                    ...
                    // 탭 클릭 시 해당 화면 표시
                    const navLabel = item.querySelector('.mobile-nav-label').textContent;
                    if (navLabel === '설정') {
                      this.showSettingsScreen(menuContainer);
                    } else if (navLabel === '구독') {
                      this.showSubscriptionScreen(menuContainer);
                    } else {
                      this.showChatList(menuContainer);
                    }
                  });
                });
         ...
         * </pre>
         * 
         * @param {HTMLElement} menuContainer 메뉴 컨테이너
         */
        showSubscriptionScreen(menuContainer) {
            const chatList = menuContainer.querySelector('.mobile-chat-list');
            const searchArea = menuContainer.querySelector('.mobile-search');
            const headerTitle = menuContainer.querySelector('.header-title');
            const headerSubtitle = menuContainer.querySelector('.header-subtitle');
            
            // 헤더 제목과 부제목 변경
            if (headerTitle) {
                headerTitle.textContent = '구독 관리';
            }
            if (headerSubtitle) {
                headerSubtitle.textContent = '스마트위험성평가 Chatbot';
            }
            ...
            chatList.innerHTML = `
                <div class="settings-screen show">
                    <div class="settings-section">
                        <h3 class="settings-section-title">구독 정보</h3>
                        <div class="settings-divider"></div>
                        <div class="settings-item">
                            <div class="settings-item-info">
                                <div class="settings-item-title">스마트위험성평가GPT</div>
                                <div class="settings-item-description">현재 버전 베타버전</div>
                                <div class="settings-item-note">*베타버전 기간동안은 무료로 사용 하실수 있습니다.</div>
                            </div>
                            <button class="settings-item-action settings-subscribe-btn" onclick="window.chatbotApp.handleSubscribe()">
                                구독
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="settings-section">
                        <h3 class="settings-section-title">구독 혜택</h3>
                        <div class="settings-divider"></div>
                        
                        <div class="settings-item">
                            <div class="settings-item-info">
                                <div class="settings-item-title">무료버전</div>
                                <div class="settings-item-description">
                                    • 챗봇 서비스 제공 (계정당 1일 5회 제공)<br>
                                    • AI성능: 기본적인 답변 제공
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="settings-item">
                            <div class="settings-item-info">
                                <div class="settings-item-title">고급버전</div>
                                <div class="settings-item-description">
                                    • 챗봇 서비스 무제한 제공<br>
                                    • AI 문서 작성 보조 서비스 제공<br>
                                    • AI성능: 고급 답변 제공
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="settings-item">
                            <div class="settings-item-info">
                                <div class="settings-item-title">전문가버전</div>
                                <div class="settings-item-description">
                                    • 챗봇 서비스 무제한 제공<br>
                                    • AI 문서 작성 보조 서비스 제공<br>
                                    • 이미지 분석을 통한 AI 위험성평가 제공<br>
                                    • AI성능: 전문가 수준 답변 제공
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="settings-item">
                            <div class="settings-item-info">
                                <div class="settings-item-title">구독 문의</div>
                                <div class="settings-item-description">
                                    구독문의는 영업팀에 문의하여 주시길 바랍니다.
                                </div>
                            </div>
                        </div>
                    </div>
         ...
         * @param {HTMLElement} menuContainer 메뉴 컨테이너
         */
        showSettingsScreen(menuContainer) {
            const chatList = menuContainer.querySelector('.mobile-chat-list');
            const searchArea = menuContainer.querySelector('.mobile-search');
            const headerTitle = menuContainer.querySelector('.header-title');
            const headerSubtitle = menuContainer.querySelector('.header-subtitle');
            ...
            chatList.innerHTML = `
                <div class="settings-screen show">
                    <div class="settings-section">
                        <h3 class="settings-section-title">데이터 관리</h3>
                        <div class="settings-divider"></div>
                        <div class="settings-item">
                            <div class="settings-item-info">
                                <div class="settings-item-title">채팅내역 초기화</div>
                                <div class="settings-item-description">모든 채팅방의 대화 내용을 삭제합니다</div>
                            </div>
                            <button class="settings-item-action" onclick="window.chatbotApp.showClearHistoryPopup()">
                                초기화
                            </button>
                        </div>
                    </div>
                ...
                <!-- 채팅내역 초기화 팝업 -->
                <div class="clear-history-popup" id="clear-history-popup">
                    <div class="popup-overlay"></div>
                    <div class="popup-content">
                        <div class="popup-header">
                            <h3 class="popup-title">주의</h3>
                        </div>
                        <div class="popup-body">
                            <p class="popup-text">이 작업은 되돌릴 수 없습니다.</p>
                            <p class="popup-text">모든 채팅방의 대화 내용이 영구적으로 삭제됩니다.</p>
                        </div>
                        <div class="popup-footer">
                            <button class="popup-btn popup-btn-cancel" onclick="window.chatbotApp.hideClearHistoryPopup()">
                                취소
                            </button>
                            <button class="popup-btn popup-btn-confirm" onclick="window.chatbotApp.clearAllChatHistory()">
                                동의
                            </button>
                        </div>
                    </div>
         ...
         * @param {HTMLElement} menuContainer 메뉴 컨테이너
         */
        showChatList(menuContainer) {
            // 설정 화면에서 채팅 목록으로 돌아가기 (메뉴는 그대로 유지)
            const chatList = menuContainer.querySelector('.mobile-chat-list');
            const searchArea = menuContainer.querySelector('.mobile-search');
            const headerTitle = menuContainer.querySelector('.header-title');
            const headerSubtitle = menuContainer.querySelector('.header-subtitle');
         ...
         * </pre>
         * 
         * @returns {HTMLElement} 메뉴 컨테이너
         */
        getMenuContainer() {
            return document.getElementById('floating-menu');
        }
        ...
        showClearHistoryPopup() {
            const popup = document.getElementById('clear-history-popup');
            if (popup) {
        ...
        clearAllChatHistory() {
            ...
            // 팝업 숨기기
            this.hideClearHistoryPopup();
            
            // 알림 표시
            this.showNotification('모든 채팅내역이 초기화되었습니다.');
         ...
         * </pre>
         * 
         * @param {HTMLElement} chatList 채팅 목록 컨테이너
         */
        renderChatList(chatList) {
            ...
            chatList.innerHTML = chatListData.map((item, index) => {
                const isMinimized = this.chatLayers[item.id] && this.chatLayers[item.id].isMinimized;
                const lastTime = getLastMessageTime(item.id);
        ...
        handleSubscribe() {
            // 구독 기능 구현 (현재는 알림만 표시)
            this.showNotification('구독 기능은 베타 버전에서 무료로 제공됩니다.');
        }
        ...
        createChatLayer(type) {
            ...
            chatLayer.innerHTML = `
                ...
                <div class="chat-input-area disabled">
                    ...
                    <button class="chat-subscribe-btn" onclick="window.chatbotApp.showSubscriptionFromChat('${type}')" aria-label="구독">
                        <i class="mdi mdi-star-outline subscribe-icon"></i>
                    </button>
                </div>
                ` : `
