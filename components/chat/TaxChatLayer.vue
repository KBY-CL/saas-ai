<!--
  [세금계산서 A.I 채팅 레이어 컴포넌트]
  - 이미지 디자인에 맞춰 챗봇 응답 메시지를 주황색 헤더바 카드 형태로 변경
-->
<template>
  <v-dialog
    v-if="!isMinimized"
  ...
  >
    <v-card :class="['modern-chatbot-window', sizeClass, 'pa-0']" :style="{ opacity: chatbotOpacity }">
      ...
      <v-card-title class="modern-header d-flex justify-space-between align-center px-4" style="padding-top: 19px; padding-bottom: 19px;">
        <div class="modern-header-left d-flex align-center" style="gap: 20px;">
          <span class="modern-header-icon">
            <v-icon color="#ff9800">mdi-robot</v-icon>
          </span>
          <div class="modern-header-title-wrap d-flex flex-column align-start">
            <span class="modern-title">세금계산서 A.I</span>
                  ...
                  <div class="modern-bot-bottom">
                    ...
                    <div class="modern-rating-container" v-if="!msg.isInitial">
                      <div class="modern-stars">
                        ...
                        >
                          <v-icon size="16" :color="star <= (msg.rating || 0) ? '#ff9800' : '#ff9800'">
      ...
      <v-card-actions class="modern-input-wrap px-3 py-2">
        <v-textarea
          v-model="input"
          label="세금계산서, 세무 관련 등 궁금한 것을 물어보세요..."
          hide-details
          auto-grow
          rows="1"
          rounded
        ...
        <v-btn
          :color="isListening ? 'orange' : undefined"
          @click="toggleMic"
  ...
  <div v-if="isMinimized" class="minimized-chatbot" @click="restoreChat" :style="minimizedStyle">
    <div class="minimized-content">
      <v-icon color="#fff">mdi-robot</v-icon>
      <span class="minimized-text">세금계산서 전문가</span>
...
function updateTransparency(value: number) {
  ...
  // 로컬 스토리지에 투명도 저장
  localStorage.setItem('taxChatbotTransparency', value.toString())
}
...
function minimizeChat() {
  ...
  // 최소화 상태를 로컬 스토리지에 저장
  localStorage.setItem('taxChatbotMinimized', 'true')
  
  // 최소화된 채팅창 목록에 추가 (세 번째 위치에 추가)
  const minimizedChatbots = JSON.parse(localStorage.getItem('minimizedChatbots') || '[]')
  
  // 기존에 있으면 제거
  const existingIndex = minimizedChatbots.indexOf('tax-ai')
  if (existingIndex > -1) {
    minimizedChatbots.splice(existingIndex, 1)
  }
  
  // 세금계산서를 배열 끝에 추가 (동적 위치)
  minimizedChatbots.push('tax-ai')
...
function restoreChat() {
  ...
  // 최소화된 채팅창 목록에서 제거
  const minimizedChatbots = JSON.parse(localStorage.getItem('minimizedChatbots') || '[]')
  const index = minimizedChatbots.indexOf('tax-ai')
  if (index > -1) {
...
function updateSlotPosition() {
  const slotIndex = minimizedChatbotsState.value.indexOf('tax-ai')
  ...
  console.log('세금계산서 인덱스:', slotIndex)
  console.log('세금계산서 슬롯:', currentSlot.value)
...
const minimizedStyle = computed(() => {
  ...
  console.log('세금계산서 슬롯:', currentSlot.value)
  console.log('세금계산서 위치:', rightPosition)
...
const sendBtnIconColor = computed(() => {
  ...
  if (isSendBtnHovered.value) return '#ff9800'
  return '#666'
})
...
function onSend() {
  ...
  setTimeout(() => {
    ...
    const aiMessage = {
      ...
      content: '세금계산서에 대한 질문을 해주셨네요. 구체적인 상황을 알려주시면 더 정확한 계산을 도와드릴 수 있습니다.',
      isUser: false,
      sentAt: botSentAt
    }
...
function rateMessage(messageId: string, rating: number) {
  ...
  if (message && !message.isUser) {
    ...
    // 로컬 스토리지에 별점 저장
    const ratings = JSON.parse(localStorage.getItem('tax_chatbot_ratings') || '{}')
    ratings[messageId] = rating
    localStorage.setItem('tax_chatbot_ratings', JSON.stringify(ratings))
...
onMounted(() => {
  // 투명도 복원
  const savedTransparency = localStorage.getItem('taxChatbotTransparency')
  if (savedTransparency !== null) {
    transparency.value = parseInt(savedTransparency)
  }
  
  // 최소화 상태 복원
  const savedMinimized = localStorage.getItem('taxChatbotMinimized')
...
<style scoped>
/* 세금계산서 챗봇 스타일 - ChatLayer와 동일한 디자인, 주황색 테마 */
...
.modern-input-wrap {
  ...
  box-shadow: 0 -2px 8px rgba(255, 152, 0, 0.04);
  min-height: 64px;
  display: flex;
  align-items: center;
  gap: 10px;
...
:deep(.modern-input .v-field__underlay) {
  ...
  --v-field-border-color: #ff9800 !important;
}
...
.minimized-chatbot:hover {
  ...
  box-shadow: 0 10px 28px rgba(255, 152, 0, 0.3);
